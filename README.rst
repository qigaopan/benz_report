==================================================================
Benz_Report: automated reports for benz project
==================================================================
...
The boilerplate code provides a dummy ``main`` function that prints out the word 'Hello'::
    ...
    >> from benz_report import main
    >> main()
    
When the package is installed via ``easy_install`` or ``pip`` this function will be bound to the ``Benz_Report`` executable in the Python installation's ``bin`` directory (on Windows - the ``Scripts`` directory).
